...
class MainWindow(QMainWindow):
    ...
    def _build_machine_group(self):
        ...
        self.stepdown_spin = self._spin(0.0, 100.0, 0.0)
        self.stepdown_spin.setSingleStep(0.5)
        self.stepdown_spin.setSpecialValueText("Off (single pass)")
        ...
        form.addRow("Max depth per pass:", self.stepdown_spin)
    ...
    def _build_info_group(self):
        ...
        form.addRow("Passes:", self.passes_label)
    ...
    def _connect_signals(self):
        ...
        self.stepdown_spin.valueChanged.connect(self.update_preview)
    ...
    def update_preview(self):
        ...
        pass_depths = compute_pass_depths(
            thickness, z_down, self.stepdown_spin.value())
        depth_each = plunge / len(pass_depths) if pass_depths else plunge
        self.passes_label.setText(f"{len(pass_depths)} × {depth_each:.2f} mm")
        try:
    ...
    def get_config(self):
        return {
            ...
            "machine": {
                ...
                "max_stepdown_mm": self.stepdown_spin.value(),
    ...
    def apply_config(self, config):
        ...
        try:
            ...
            self.stepdown_spin.setValue(
                float(machine.get("max_stepdown_mm", self.stepdown_spin.value()))
            )
    ...
    def build_gcode(self):
        ...
        max_stepdown = self.stepdown_spin.value()
        pass_depths = compute_pass_depths(thickness, z_down, max_stepdown)

        passes_comment = f"; Passes: {len(pass_depths)}"
        if len(pass_depths) > 1:
            passes_comment += (
                f" ({plunge / len(pass_depths):.3f} mm each, "
                f"max stepdown {max_stepdown:.3f} mm)"
            )
